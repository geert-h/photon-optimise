...
unsafe fn sobel_horizontal_channel_simd(
    ...
    for y in 0..height {
        ...
        while x + 16 <= width - 1 {
            ...
            let center_lo = i16x8_extend_low_u8x16(center);
            let center_hi = i16x8_extend_high_u8x16(center);
            let sum_lo = i16x8_add(
                i16x8_add(i16x8_extend_low_u8x16(left), i16x8_shl(center_lo, 1)),
                i16x8_extend_low_u8x16(right),
            );
            let sum_hi = i16x8_add(
                i16x8_add(i16x8_extend_high_u8x16(left), i16x8_shl(center_hi, 1)),
        ...
        while x < width - 1 {
            scratch[row + x] = src[row + x - 1] as i16
                + 2 * src[row + x] as i16
                + src[row + x + 1] as i16;
            x += 1;
        }
...
pub(super) unsafe fn sobel_vertical_simd(
    ...
    sobel_vertical_channel_simd(&src.r, &mut dst.r, scratch, width, height);
    sobel_vertical_channel_simd(&src.g, &mut dst.g, scratch, width, height);
    sobel_vertical_channel_simd(&src.b, &mut dst.b, scratch, width, height);
    sobel_vertical_channel_simd(&src.a, &mut dst.a, scratch, width, height);
    restore_alpha_if_filter_zeroed_it(src, dst, width, height);
}
...
unsafe fn sobel_vertical_channel_simd(
    ...
    for y in 0..height {
        ...
        while x + 16 <= width - 1 {
            ...
            let sum_lo =
                i16x8_sub(i16x8_extend_low_u8x16(right), i16x8_extend_low_u8x16(left));
            let sum_hi = i16x8_sub(
                i16x8_extend_high_u8x16(right),
        ...
        while x < width - 1 {
            scratch[row + x] = src[row + x + 1] as i16 - src[row + x - 1] as i16;
            x += 1;
        }
    }
    ...
    for y in 1..height - 1 {
        ...
        while x + 16 <= width - 1 {
            ...
            let sum_lo = i16x8_add(bot_lo, i16x8_add(i16x8_shl(mid_lo, 1), top_lo));
            let sum_hi = i16x8_add(bot_hi, i16x8_add(i16x8_shl(mid_hi, 1), top_hi));
            let out = u8x16_narrow_i16x8(sum_lo, sum_hi);
        ...
        while x < width - 1 {
            let value = scratch[bot + x] as i32
                + 2 * scratch[mid + x] as i32
