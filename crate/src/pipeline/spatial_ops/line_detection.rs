...
pub(super) unsafe fn detect_horizontal_lines_simd(
    src: &PlanarImage,
    dst: &mut PlanarImage,
    scratch: &mut [i16],
) {
...
#[target_feature(enable = "simd128")]
pub(super) unsafe fn detect_vertical_lines_simd(
    src: &PlanarImage,
    dst: &mut PlanarImage,
) {
    let width = src.width() as usize;
...
//     dst.a.fill(0);
//
//     detect_vertical_lines_channel_simd(&src.r, &mut dst.r, scratch, width, height);
//     detect_vertical_lines_channel_simd(&src.g, &mut dst.g, scratch, width, height);
//     detect_vertical_lines_channel_simd(&src.b, &mut dst.b, scratch, width, height);
//     detect_vertical_lines_channel_simd(&src.a, &mut dst.a, scratch, width, height);
//     restore_alpha_if_filter_zeroed_it(src, dst, width, height);
// }
//
// #[target_feature(enable = "simd128")]
...
//             let right = v128_load(src.as_ptr().add(row + x + 1) as *const v128);
//
//             let sum_lo = i16x8_sub(
//                 i16x8_sub(
//                     i16x8_shl(i16x8_extend_low_u8x16(center), 1),
//                     i16x8_extend_low_u8x16(left),
//                 ),
//                 i16x8_extend_low_u8x16(right),
//             );
//             let sum_hi = i16x8_sub(
//                 i16x8_sub(
//                     i16x8_shl(i16x8_extend_high_u8x16(center), 1),
//                     i16x8_extend_high_u8x16(left),
//                 ),
//                 i16x8_extend_high_u8x16(right),
//             );
...
//
//         while x < width - 1 {
//             scratch[row + x] = 2 * src[row + x] as i16
//                 - src[row + x - 1] as i16
//                 - src[row + x + 1] as i16;
//             x += 1;
//         }
...
//             let bot_hi = v128_load(scratch.as_ptr().add(bot + x + 8) as *const v128);
//
//             let value_lo = i16x8_add(i16x8_add(mid_lo, top_lo), bot_lo);
//             let value_hi = i16x8_add(i16x8_add(mid_hi, top_hi), bot_hi);
//             let out = u8x16_narrow_i16x8(value_lo, value_hi);
//
//             v128_store(dst.as_mut_ptr().add(mid + x) as *mut v128, out);
//
